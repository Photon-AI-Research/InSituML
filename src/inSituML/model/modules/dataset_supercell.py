...
class PCDataset(Dataset):
    def __init__(self, item_phase_space, normalize=False, a=0.0, b=1.0):
        """
        Prepare dataset
        Args:
        ...
        if normalize:
            ...
            self.vmin_rad, self.vmax_rad = (
                torch.Tensor([0.05, 0.1]).float(),
                torch.Tensor([0.15, 0.3]).float(),
            )
    ...
    def __getitem__(self, index):
        return (self.item[index, :], torch.Tensor([0.1, 0.2]).float())
