...
def sample_pointcloud(model, num_samples, cond, vmin, vmax):
    ...
    with torch.no_grad():
        ...
        pc_pr_reshaped = denormalize_point(pc_pr_reshaped.cpu(), vmin, vmax, a=0., b=1.)
...
class Loader:
    def __init__(self, pathpattern="/data_box/60_70_130_140_0_12/{}.npy", t0=0, t1=100, timebatchsize=20, particlebatchsize=10240):
    ...
    def __getitem__(self, idx):
        ...
        class Epoch:
            ...
            def __getitem__(self, timebatch):
                ...
                for time in bi:
                    index = time + self.t0
                    p = torch.from_numpy(np.load(self.loader.pathpattern.format(index)).astype(np.float32) )
                    particles.append( p )
