...
path_to_supercells = '/bigdata/hplsim/aipp/Anna/lwfa_2cells_prevm/'
...
for ind, series in enumerate(all_series[:1]):
            ...
            for supercell_num_y in range(-1*num_neighbours,num_neighbours):
                for supercell_num_z in range(-1*num_neighbours,num_neighbours):
                    ...
                    supercell_file = open(path_to_supercells
                                            +'_'.join([str(iteration), str(xnum + supercell_num_x), str(ynum + supercell_num_y), str(znum + supercell_num_z)])+'.npy', 'wb')
    ...
    for chunk_num in range(100000000):
            ...
            for supercell_num_x in range(-1*num_neighbours,num_neighbours):
                for supercell_num_y in range(-1*num_neighbours,num_neighbours):
                    for supercell_num_z in range(-1*num_neighbours,num_neighbours):
                        ...
                        #print(my_filter.shape, x_pos_offset.shape)
                        supercell_filters.append((path_to_supercells
                                            +'_'.join([str(iteration), str(xnum + supercell_num_x), str(ynum + supercell_num_y), str(znum + supercell_num_z)])+'.npy', my_filter))
        '''
        x_pos = None
        y_pos = None
        z_pos = None
        ...
        for supercell_num in range(len(supercell_filters)):
            #print('Macrocell: ', supercell_num)
            #print('Shape of mask: ', supercell_filters[supercell_num].shape)
            #print('Matched particles: ', supercell_filters[supercell_num].sum())

            x_pos_ = x_pos[supercell_filters[supercell_num][1]]
            y_pos_ = y_pos[supercell_filters[supercell_num][1]]
            z_pos_ = z_pos[supercell_filters[supercell_num][1]]

            x_pos_offset_ = x_pos_offset[supercell_filters[supercell_num][1]]
            y_pos_offset_ = y_pos_offset[supercell_filters[supercell_num][1]]
            z_pos_offset_ = z_pos_offset[supercell_filters[supercell_num][1]]

            x_momentum_ = x_momentum[supercell_filters[supercell_num][1]]
            y_momentum_ = y_momentum[supercell_filters[supercell_num][1]]
            z_momentum_ = z_momentum[supercell_filters[supercell_num][1]]
            
            x_momentum_prev_ = x_momentum_prev[supercell_filters[supercell_num][1]]
            y_momentum_prev_ = y_momentum_prev[supercell_filters[supercell_num][1]]
            z_momentum_prev_ = z_momentum_prev[supercell_filters[supercell_num][1]]
            ...
            if particle_tensor.shape[0] > 0:
                ...
                supercell_tensors.append((supercell_filters[supercell_num][0], particle_tensor))
            particle_tensor = None
    
    for supercell in range(len(supercell_tensors)):
        print('Supercell: '+supercell_tensors[supercell][0])
        supercell_file = open(supercell_tensors[supercell][0], 'wb')
        np.save(supercell_tensors[supercell][0], supercell_tensors[supercell][1])
        supercell_file.close()
