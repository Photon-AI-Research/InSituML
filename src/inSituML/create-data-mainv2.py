...
for ind, series in enumerate(all_series):
    ...
    h2 = particles.particle_patches["numParticles"][
        io.Mesh_Record_Component.SCALAR
    ]
    numParticles = h2.load()

    h3 = particles.particle_patches["numParticlesOffset"][
        io.Mesh_Record_Component.SCALAR
    ]
    ...
    for id_gpu in range(numParticlesOffset.shape[0]):
        ...
        x_pos = particles["position"]["x"][
            numParticlesOffset[id_gpu] : numParticlesOffset[id_gpu]
            + numParticles[id_gpu]
        ]  # [:num_particles]
        ...
        particle_tensor = particle_tensor[
            ~np.isnan(particle_tensor).any(axis=1)
        ]
        print(
    ...
    file_path = (
        "/bigdata/hplsim/aipp/Jeyhun/khi/part_rad/particle_002/"
        + str(iteration)
        + ".npy"
    )
