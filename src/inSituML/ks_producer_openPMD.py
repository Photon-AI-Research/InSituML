...
class RandomLoader(Thread):
    ...
    def __init__(
        ...
        self.t1 = hyperParameterDefaults[
            "t1"
        ]  # endpoint=false, t1 is not loaded from disk
        ...
        self.particlePerGPU = hyperParameterDefaults[
            "number_particles_per_gpu"
        ]
        self.amplitude_direction = hyperParameterDefaults[
            "amplitude_direction"
        ]
    ...
    def run(self):
        ...
        radiationSeries = opmd.Series(
            self.radiationPathPattern, opmd.Access.read_only
        )
        ...
        while i_epoch < np.ceil(self.numEpochs):
            ...
            for step in perm + self.t0:
                ...
                numParticles = ps.particle_patches["numParticles"][
                    opmd.Mesh_Record_Component.SCALAR
                ].load()
                numParticlesOffsets = ps.particle_patches[
                    "numParticlesOffset"
                ][opmd.Mesh_Record_Component.SCALAR].load()
                series.flush()
                ...
                randomParticlesPerGPU = np.array(
                    [
                        self.rng.choice(
                            numParticles[i], self.particlePerGPU, replace=False
                        )
                        for i in np.arange(len(numParticles))
                    ]
                )
                randomParticles = np.array(
                    [
                        randomParticlesPerGPU[i] + numParticlesOffsets[i]
                        for i in np.arange(len(numParticles))
                    ]
                )

                local_region = {
                    "offset": [numParticlesOffsets[0]],
                ...
                for i_c, component in enumerate(["x", "y", "z"]):
                    ...
                    gpuBoxExtent = ps.particle_patches["extent"][
                        component
                    ].load()
                    gpuBoxOffset = ps.particle_patches["offset"][
                        component
                    ].load()
                    series.flush()

                    if "position" in self.reqPhaseSpaceVars:
                        pos = ps["position"][component].load_chunk(
                            local_region["offset"], local_region["extent"]
                        )
                        posOffset = ps["positionOffset"][component].load_chunk(
                            local_region["offset"], local_region["extent"]
                        )
                        series.flush()
                        ...
                        loaded_particles[writing_index + i_c] = torch_stack(
                            [
                                torch_from_numpy(position[r])
                                for r in randomParticles
                            ]
                        )
                        del position
                    ...
                    if (
                        ...
                        momentum = ps["momentum"][component].load_chunk(
                            local_region["offset"], local_region["extent"]
                        )
                        series.flush()
                        loaded_particles[writing_index + i_c] = torch_stack(
                            [
                                torch_from_numpy(momentum[r])
                                for r in randomParticles
                            ]
                        )
                        writing_index += 3
                    ...
                    if "force" in self.reqPhaseSpaceVars:
                        momentumPrev1 = ps["momentumPrev1"][
                            component
                        ].load_chunk(
                            local_region["offset"], local_region["extent"]
                        )
                        ...
                        momPrev1_reduced = torch_stack(
                            [
                                torch_from_numpy(momentumPrev1[r])
                                for r in randomParticles
                            ]
                        )
                    ...
                    if "momentum" in self.reqPhaseSpaceVars:
                        for particleBoxIndex in np.arange(len(numParticles)):
                            loaded_particles[
                                ...
                                - self.normalization["momentum_mean"]
                            ) / self.normalization[
                                "momentum_std"
                            ]
                ...
                distributed_amplitudes = torch_empty(
                    (3, len(r_offset), len(DetectorFrequency)),
                    dtype=torch_complex64,
                )  # shape: (components, GPUs, frequencies)

                for i_c, component in enumerate(["x", "y", "z"]):
                    Dist_Amplitude = radIter.meshes["Amplitude_distributed"][
                        component
                    ].load_chunk()  # shape: (GPUs, directions, frequencies)
                    # MAGIC: only look along one direction
                    DetectorDirection = radIter.meshes["DetectorDirection"][
                        component
                    ][
                        :, 0, 0
                    ]  # shape: (x directions)
                    ...
                    r_offset[:, i_c] = gpuBoxOffset * iteration.get_attribute(
                        cellExtensionNames[component]
                    )
                ...
                )  # shape: (GPUs, components, frequencies)
                ...
                while True:
                    try:
                        self.data.put(
                            [loaded_particles, distributed_amplitudes],
                            block=False,
                        )
                        break
                    except Full:
