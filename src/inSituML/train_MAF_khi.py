...
class Loader:
    def __init__(self, pathpattern="/bigdata/hplsim/aipp/Jeyhun/khi/particle/{}.npy", t0=0, t1=100, timebatchsize=20, particlebatchsize=10240):
        ...
        num_files = t1 - t0
        missing_files = [i for i in range(t0, t1) if not os.path.exists(pathpattern.format(i))]
    ...
    def __getitem__(self, idx):
        ...
        class Epoch:
            def __init__(self, loader, timebatchsize=20, particlebatchsize=10240):
            ...
            def __getitem__(self, timebatch):
                ...
                particles = []
                for time in bi:
                    p = torch.from_numpy(np.load(self.loader.pathpattern.format(time)).astype(np.float32) )
                    particles.append( p )
...
if __name__ == "__main__":
    ...
    if arg1 == 'checkpoint':
        ...
        checkpoint_path = '/bigdata/hplsim/aipp/Jeyhun/khi/checkpoints/6d7p03r1/model_1052'
    ...
    for i_epoch in range(start_epoch, hyperparameter_defaults["num_epochs"]):   
        ...
        for tb in range(len(epoch)):
            ...
            for b in range(len(timebatch)):
                ...
                phase_space = normalize_point(phase_space, pos_minmax[0], pos_minmax[1], a=0., b=1.)
                
                # for param in model.model.parameters():
                #     param.grad = None
                
                loss = - model.model.log_prob(inputs=phase_space.to(model.device),context=times.to(model.device))
