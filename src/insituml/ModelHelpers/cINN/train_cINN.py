...
from insituml.StreamDataReader.async_stream_buffer import AsyncStreamBuffer
...
use_streaming = path_to_particle_data.endswith('.sst')
if use_streaming:
    assert path_to_radiation_data.endswith('.sst')

    dataset_type = streamed_dataset.StreamedPCDataset
    stream_reader = AsyncStreamBuffer(
        path_to_particle_data,
        os.path.join(
            '..',
            '..',
            'StreamDataReader',
            'pc_stream_config.json',
        ),
    )
    print('created pc stream')

    radiation_stream_reader = AsyncStreamBuffer(
        path_to_radiation_data,
        os.path.join(
            '..',
            '..',
            'StreamDataReader',
            'radiation_stream_config.json',
        ),
    )
    print('created rad stream')
    dataset_tr_kwargs = dict(
        stream_reader=stream_reader,
        radiation_stream_reader=radiation_stream_reader,
    )
    dataset_val_kwargs = dict(
        stream_reader=path_to_particle_data,
        radiation_stream_reader=path_to_radiation_data,
    )
else:
    ...
    dataset_type = dataset.PCDataset
    dataset_tr_kwargs = dict(
...
dataset_tr = dataset_type(**dataset_tr_kwargs,
                          num_points=-1,
...
if hasattr(dataset_tr, 'shape_ps'):
    print(dataset_tr.shape_ps)
    print(dataset_tr.shape_rad)

    ps_dim = dataset_tr.shape_ps[-1]
    radiation_dim = dataset_tr.shape_rad[-1]
else:
    ps_dim = 0
    radiation_dim = 0
