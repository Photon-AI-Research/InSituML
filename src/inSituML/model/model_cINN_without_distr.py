    ...
    def train_(
        ...
        for i_epoch in range(epochs):
            ...
            for phase_space, radiation in loader_tr:
                ...
                # optimizer.zero_grad()
                z, log_j = self(
                    phase_space.to(self.device), radiation.to(self.device)
                )
                loss = 0.0
                ...
                torch.nn.utils.clip_grad_norm_(self.trainable_parameters, 10.0)
            ...
            if self.enable_wandb:
                wandb.log(
                    {
                        "loss_z_avg_tr": sum(loss_z) / len(loss_z),
    ...
    def validation(self, dataset_val, batch_size, normalize):
        ...
        with torch.no_grad():
            ...
            if self.enable_wandb:
                wandb.log(
                    {
                        "loss_z_val": sum(loss_z) / len(loss_z),
