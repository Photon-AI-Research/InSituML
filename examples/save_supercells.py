...
path_to_supercells = '/bigdata/hplsim/aipp/Anna/lwfa_tests1/'
...
for ind, series in enumerate(all_series[:1]):
    ...
    for supercell_num in range(num_supercells):
        print('Supercell: '+'; '.join([str(xnum[supercell_num]), str(ynum[supercell_num]), str(znum[supercell_num])]))
        supercell_file = open(path_to_supercells
                                +'_'.join([str(iteration), str(xnum[supercell_num]), str(ynum[supercell_num]), str(znum[supercell_num])])+'.npy', 'wb')
    ...
    for chunk_num in range(100000000):
        ...
        for supercell_num in range(num_supercells):
            ...
            my_filter = np.logical_and(np.logical_and((x_pos_offset // 8 == xnum[supercell_num]),
                                                      (y_pos_offset // 8 == ynum[supercell_num])),
                                       z_pos_offset // 4 == znum[supercell_num])
            '''
    ...
    for supercell_num in range(num_supercells):
        print('Close file ', supercell_files[supercell_num])
