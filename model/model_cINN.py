        ...
        for i_epoch in range(epochs):
            ...
            for phase_space, radiation in loader_tr:
                if dataset_tr.normalize:
                    ...
                    '''
                    batch_num_column_ps = torch.zeros((phase_space.shape[0]*phase_space.shape[1],1))
                    for num_batch in range(phase_space.shape[0]):
                        if batch_num_column[num_batch*phase_space.shape[1]:,0].shape[0] < (num_batch+1)*phase_space.shape[1]:
                            batch_num_column[num_batch*phase_space.shape[1]:(num_batch+1)*phase_space.shape[1], 0] = num_batch
                        else:
                            batch_num_column[num_batch*phase_space.shape[1]:, 0] = num_batch
                    phase_space = phase_space.reshape(phase_space.shape[0]*phase_space.shape[1],
                                                            phase_space.shape[2])
                    '''
                    radiation = data_preprocessing.normalize_point(radiation, dataset_tr.vmin_rad, dataset_tr.vmax_rad)

                print(phase_space.shape, radiation.shape)
                #print(radiation.reshape(radiation.shape[1]*radiation.shape[2]*radiation.shape[3]).shape)
                #print(440*2048*6)
