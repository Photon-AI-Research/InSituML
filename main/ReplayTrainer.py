...
class ReplayTrainer(ModelTrainer, EpisodicMemory):
    ...
    def __init__(self, model_path, model_loss_func, input_channels, number_model_layers, number_conv_layers, filters, latent_size, epochs, learning_rate, run_name, input_sizes, number_of_tasks, dataset_name, classes, saveModelInterval, replayer_mem_size, aGEM_selection_size, store_encoded, layerWise = False ,model_type=..., e_field_dimension=None, is_e_field=False, activation="leaky_relu", optimizer="adam",batch_size=3, onlineEWC=False, ewc_lambda=0, gamma=0, mas_lambda=0,  agem_l_enc_lambda = 1):
        ModelTrainer.__init__(self, model_path, model_loss_func, input_channels, number_model_layers, number_conv_layers, filters, latent_size, epochs, learning_rate, run_name, input_sizes, number_of_tasks, dataset_name, classes, saveModelInterval, model_type=model_type, e_field_dimension=e_field_dimension, is_e_field=is_e_field, activation=activation, optimizer=optimizer, batch_size=batch_size, onlineEWC=onlineEWC, ewc_lambda=ewc_lambda, gamma=gamma, mas_lambda=mas_lambda,  agem_l_enc_lambda = agem_l_enc_lambda)
