    ...
    def __call__(self, losses, batch_index):
        ...
        if not self.printedHeader:
            self.out_stream.write(
                "\t".join(["# batch_index", "time"] + list(losses.keys()))
                + "\n"
            )
        ...
        self.out_stream.write(
            "\t".join(
                [str(batch_index), str(current - self.last_time_point)]
                + list(str(v.item()) for v in losses.values())
            )
            + "\n"
        )
...
class ModelTrainer(Thread):
    ...
    def __init__(
        ...
        if gpu_id is not None:
            ...
            self.gpu_id = torch.device(
                "cuda" if torch.cuda.is_available() else "cpu"
            )
            # should we not still push the model there?

        self.logger = LossLogger(
            logger, max_logs=max_logs, out_prefix=out_prefix
        )
    ...
    def run(self):
        ...
        while True:
            ...
            if self.batch_passes > 0:
                ...
                if (
                    ...
                    and self.batch_passes % self.checkpoint_interval == 0
                ):
                    save_checkpoint_conditionally(
                        self.model,
                        self.optimizer,
                        self.out_prefix,
                        self.batch_passes,
            ...
            if self.training_buffer.openpmdProduction == False:
                ...
                if (
                    rest_training_left_counter
                    > self.ts_after_stopped_production
                ):
                    if self.batch_passes > 0:
                        self.logger(
                            losses, self.batch_passes - 1
                        )  # log last batch
                        if self.checkpoint_final or (
                            self.checkpoint_interval
                            and self.batch_passes % self.checkpoint_interval
                            == 0
                        ):
                            save_checkpoint_conditionally(
                                self.model,
        ...
        print(
            "Training ended after {} samples in {} batches.".format(
                self.training_samples, self.batch_passes
            ),
            flush=True,
        )
