...
def get_data_phase_space(ind, items, num_particles=-1, species="e_all"):
    """
    ...
    series = io.Series(
        items[ind], io.Access.read_only, '{"defer_iteration_parsing": true}'
    )
    # print(items[ind].split('.bp')[-1].split('_')[-1])
    i = series.iterations[
        int(items[ind].split(".bp")[0].split("_")[-1])
    ].open()
    ...
    if num_particles == -1:
        ...
        inds = random.sample(
            list(range(0, particle_tensor.shape[0])), num_particles
        )
        return torch.from_numpy(particle_tensor[inds, :]).float()


def get_phase_space_by_chunks(ind, items, chunk_size=100, species="e_all"):
    """
    Load file with 1 particle cloud
    Args:
        ind(integer): number of path in list of paths to files
    ...
    series = io.Series(
        filename, io.Access.read_only, '{"defer_iteration_parsing": true}'
    )
...
def get_testdata_radiation(ind, items, chunk_size):
    """
...
def get_radiation_spectra(ind, items, chunk_size):
    """
    ...
    spectra = torch.from_numpy(
        (radiation.RadiationData(items[ind])).get_Spectra()
    ).float()
    return torch.flatten(spectra.repeat(chunk_size, 1, 1), start_dim=1)


def get_radiation_spectra_intergrated_over_directions(ind, items, chunk_size):
    """
...
def get_radiation_spectra_intergrated_over_frequencies(ind, items, chunk_size):
    """
...
def get_unit_condition(ind, items, chunk_size):
    """
...
def get_radiation_spectra_2_projections(ind, items, chunk_size):
    """
    ...
    """
    spectra = (radiation.RadiationData(items[ind])).get_Spectra()
    concatenated_projections = torch.from_numpy(
        np.concatenate((np.sum(spectra, axis=0), np.sum(spectra, axis=1)))
    ).float()
...
def denormalize_point(point, vmin, vmax, a=0.0, b=1.0):
    """
...
def get_vmin_vmax_radiation(items, chunk_size, get_radiation_data):
    """
...
def get_vmin_vmax_radiation_np(items, chunk_size, get_radiation_data):
    """
    Find minima/maxima in all radiation simulations for normalization
    Args:
        items(list of string): list of paths to all electron clouds
...
def get_shape(item, species):
    series = io.Series(
        item, io.Access.read_only, '{"defer_iteration_parsing": true}'
    )
