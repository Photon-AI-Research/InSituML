...
def plot_3D(particle_tensor, color_component=-1, return_fig=True):
    ...
    norm = matplotlib.colors.Normalize(
        vmin=np.min(particle_tensor[:, comp]),
    ...
    ax.scatter(
        ...
        particle_tensor[:, 1],
        particle_tensor[:, 2],
        c=plt.cm.jet(norm(particle_tensor[:, comp])),
        s=20,
        alpha=0.5,
    )
    ax.view_init(40, 200)
...
def plot_per_slice(
    ...
    slices = [
        np.min(pc[:, slice_along])
        + (np.max(pc[:, slice_along]) - np.min(pc[:, slice_along]))
        * i
        / num_slices
    ...
    for ind in range(len(slices) - 1):
        pc_[:, -1][
            (pc_[:, slice_along] >= slices[ind])
            & (pc_[:, slice_along] <= slices[ind + 1])
        ] = ind
    ...
    if comp_of_interest is not None:
        mean_energy = [
            (
                np.mean(pc_[:, comp_of_interest][pc_[:, -1] == ind])
                if (pc_[:, comp_of_interest][pc_[:, -1] == ind]).shape[0] > 1
                else None
            )
    ...
    if comp_of_interest == None:
        num_particles = [
            pc_[pc_[:, -1] == ind].shape[0] for ind in range(len(slices))
        ]
...
def plot_per_slice_GTandRec(
    ...
    if pc_pred is not None:
        slices_pred = [
            np.min(pc_pred[:, slice_along])
            + (
                np.max(pc_pred[:, slice_along])
                - np.min(pc_pred[:, slice_along])
            )
            * i
            / num_slices
        ...
        pc_pred_ = np.concatenate(
            (pc_pred, np.zeros((pc_pred.shape[0], 1))), axis=1
        )

        for ind in range(len(slices_pred) - 1):
            pc_pred_[:, -1][
                (pc_pred_[:, -3] >= slices_pred[ind])
                & (pc_pred_[:, -3] <= slices_pred[ind + 1])
            ] = ind
        pc_pred_[:, -1][(pc_pred_[:, -3] >= slices_pred[-1])] = (
            len(slices_pred) - 1
        )
    ...
    if comp_of_interest is not None:
        ...
        std_energy = [
            (
                np.std(pc_[:, comp_of_interest][pc_[:, -1] == ind])
                if pc_[:, comp_of_interest][pc_[:, -1] == ind].shape[0] > 1
                else None
            )
            for ind in range(len(slices))
        ...
        axs.plot(
            [slice_ for slice_ in slices], mean_energy, label="Groundtruth"
        )
        if pc_pred is not None:
            mean_energy_pred = [
                (
                    np.mean(
                        pc_pred_[:, comp_of_interest + 1][
                            pc_pred_[:, -1] == ind
                        ]
                    )
                    if (
                ...
                for ind in range(len(slices_pred))
            ]
            std_energy_pred = [
                (
                    np.std(
                    ...
                    if pc_pred_[:, comp_of_interest + 1][
                        pc_pred_[:, -1] == ind
                    ].shape[0]
                    > 1
                    else None
            ...
            axs.plot(
                [slice_ for slice_ in slices_pred],
                mean_energy_pred,
    ...
    if comp_of_interest == None:
        ...
        if pc_pred is not None:
            num_particles_pred = [
                pc_pred_[pc_pred_[:, -1] == ind].shape[0]
                for ind in range(len(slices_pred))
            ]
...
def plot_3D_GTandRec(pc_groundtruth, pc_reconstructed, color_component=-1):
    ...
    norm = matplotlib.colors.Normalize(
        vmin=np.min(pc_groundtruth[:, comp]),
    ...
    ax.scatter(
        ...
        pc_groundtruth[:, 1],
        pc_groundtruth[:, 2],
        c=plt.cm.jet(norm(pc_groundtruth[:, comp])),
        s=20,
        alpha=0.5,
    )
    ax.view_init(40, 200)
    ...
    if pc_reconstructed is not None:
        ...
        norm = matplotlib.colors.Normalize(
            vmin=np.min(pc_reconstructed[:, comp]),
        ...
        ax.scatter(
            ...
            pc_reconstructed[:, 1],
            pc_reconstructed[:, 2],
            c=plt.cm.jet(norm(pc_reconstructed[:, comp])),
            s=20,
            alpha=0.5,
        )
        ax.view_init(40, 200)
...
def log_min_max(test_pointcloud, test_radiation, model):
    ...
    supercell = [
        int(k)
        for k in test_pointcloud.split("/")[-1].split(".")[0].split("_")[1:]
    ]
    iteration = test_pointcloud.split("/")[-1].split(".")[0].split("_")[0]

    radiation_tensor = data_preprocessing.get_radiation_spectra_2_projections(
        0, [test_radiation], num_particles
    )
    vmin_rad, vmax_rad = np.load(path_to_minmax + "vmin_rad.npy"), np.load(
        path_to_minmax + "vmax_rad.npy"
    )

    radiation_tensor = data_preprocessing.normalize_point(
        radiation_tensor,
        torch.full(radiation_tensor.shape, vmin_rad[0, 0]),
        torch.full(radiation_tensor.shape, vmax_rad[0, 0]),
        torch.full(radiation_tensor.shape, 0.0),
        torch.full(radiation_tensor.shape, 1.0),
    )

    gt = np.load(test_pointcloud)

    pred_pointcloud_full = model.sample_pointcloud(
        radiation_tensor.to("cuda"), num_particles
    )
    pred_pointcloud_full = pred_pointcloud_full.detach().cpu().numpy()

    gt_filter = np.logical_and(
        (
            np.logical_and(
                np.logical_and(
                    (gt[:, 0] // 8 < supercell[0] + num_supercells_to_merge),
    ...
    mins = [
        np.min(pred_pointcloud_full[:, i])
        for i in range(pred_pointcloud_full.shape[1])
    ]
    maxs = [
        np.max(pred_pointcloud_full[:, i])
        for i in range(pred_pointcloud_full.shape[1])
    ]
    # print(supercell)
    ...
    for ind, elem in enumerate(mins):
        overlaps.append(
            getOverlap([mins_gt[ind], maxs_gt[ind]], [mins[ind], mins[ind]])
            > 0
        )
    wandb.log(
        {
            "Supercell "
            + " ".join([str(k) for k in supercell])
            + ", Overlap with prediction ": int(all(overlaps) == True)
        }
    )
...
def log_each_plot(
    ...
    pred_pointcloud_full = model.sample_pointcloud(
        radiation_tensor.to(model.device), radiation_tensor.shape[0]
    )
    pred_pointcloud_full = pred_pointcloud_full.detach().cpu().numpy()

    fig = plot_3D(
        pred_pointcloud_full[
            np.random.choice(
                pred_pointcloud_full.shape[0], 2000, replace=False
            )
        ]
    )
        ...
        (
            np.logical_and(
                np.logical_and(
                    (
                        x_pos_offset // 8
                        < xnum[supercell_num] + num_supercells_to_merge
                    ),
                    (
                        y_pos_offset // 8
                        < ynum[supercell_num] + num_supercells_to_merge
                    ),
                ),
                z_pos_offset // 4
                < znum[supercell_num] + num_supercells_to_merge,
            )
        ),
        (
            np.logical_and(
    ...
    wandb.log(
        {
            ...
            + iteration
            + ", supercell: "
            + " ".join([str(k) for k in supercell]): image
        }
    )
    plt.close()
    ...
    for i in range(1, 4, 1):
        plot_per_slice_GTandRec(
            ...
            slice_along,
            num_slices,
            comp_of_interest=-1 * i,
            axs=axs[i],
            label="Mean " + labels_y[-1 * i],
        )
    image = wandb.Image(fig)
    wandb.log(
        {
...
def log_one_plot(test_pointcloud, test_radiation, model):
    ...
    radiation_tensor = (
        torch.Tensor([0.5, 0.5]).float().repeat(pointcloud_tensor.shape[0], 1)
    )
