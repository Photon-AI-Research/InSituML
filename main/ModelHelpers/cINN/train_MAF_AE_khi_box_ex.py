...
from train_khi_AE_refactored.networks import ConvAutoencoder
...
def generate_plots_maf(model, t_index,gpu_index,pathpattern1,pathpattern2, config,normalizer, device, epoch, pbatch, plot_every):
    ...
    with torch.no_grad():
        ...
        pc_pr_ae, z_encoded = model.reconstruct_ae(p_gt_clone)
...
if __name__ == "__main__":
    ...
    class ModelFinal(nn.Module):
        ...
        def reconstruct_maf(self, num_samples, cond):
            ...
            y = self.AE.decoder(lat_z_pred)
            
            return y, lat_z_pred
        
        def reconstruct_ae(self, x):
            
            y,z_encoded = self.AE.reconstruct_input(x)
    ...
    model_ae = ConvAutoencoder(encoder = Encoder, 
          encoder_kwargs = encoder_kwargs, 
          decoder = Conv3DDecoder, 
          z_dim=config["latent_space_dims"],
          decoder_kwargs = decoder_kwargs,
          loss_function = EarthMoversLoss(),
          property_=config["property_"],
          particles_to_sample = config["particles_to_sample"],
          ae_config="simple")
    
    print('model_ae', model_ae)
    ...
    model = ModelFinal(model_ae, inner_model, EarthMoversLoss(), weight_AE=config["weight_AE"],
