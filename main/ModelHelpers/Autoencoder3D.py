...
class AutoEncoder3D(ContinualLearner):
    ...
    def _get_final_flatten_size(self):
        return self.__half_int(self.input_sizes[0],self.n_layers) * self.__half_int(self.input_sizes[1],self.n_layers) * self.__half_int(self.input_sizes[2],self.n_layers) * self.filters[self.n_layers - 1]
    ...
    def decode(self, x, pooling_indexes_list):
        ...
        x = x.view(-1,self.filters[self.n_layers - 1],self.__half_int(self.input_sizes[0],self.n_layers),self.__half_int(self.input_sizes[1],self.n_layers), self.__half_int(self.input_sizes[2],self.n_layers))
        b_size = x.size()[0]

        for i in range(self.n_layers):
            if i == self.n_layers - 1:
                op_size = torch.Size((b_size, self.filters[0],self.input_sizes[0],self.input_sizes[1], self.input_sizes[2]))
            else:
    ...
    def forward(self,x):
        ...
        encoded = to_device(encoded,'cuda:1')
        pooling_index_list = to_device(pooling_index_list, 'cuda:1')
        decoded = self.decode(encoded, pooling_index_list)
        return to_device(encoded,'cuda:0'), to_device(decoded,'cuda:0'), to_device(pooling_index_list,'cuda:0')
    
    def from_dec_to_enc(self,x):
        x = to_device(x,'cuda:1')
        #calculate reshape dimensions
        x = self.linear_layer_decoder(x)
        x = x.view(-1,self.filters[self.n_layers - 1],self.__half_int(self.input_sizes[0],self.n_layers),self.__half_int(self.input_sizes[1],self.n_layers), self.__half_int(self.input_sizes[2],self.n_layers))
        for i in range(self.n_layers):
    ...
    def save_checkpoint(self, path, name, after_task):
        ...
        try:
            ...
            pass
    def split_gpus(self):
        print("Splitting on GPU")
        device1 = 'cuda:0'
        device2 = 'cuda:1'

        # Encoder to GPU - 0
        to_device(self.enc_layers, device1)
        to_device(self.pooling_layers, device1)
        to_device(self.flatten_layer, device1)
        to_device(self.linear_layer_encoder, device1)
        
        # Decoder to GPU - 1
        to_device(self.linear_layer_decoder, device2)
        to_device(self.unpooling_layers, device2)
        to_device(self.dec_layers, device2)