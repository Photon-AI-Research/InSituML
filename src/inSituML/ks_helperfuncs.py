...
def denormalize_columns(normalized_array, gt):
    ...
    denormalized_array = concatenate(
        (xyz_columns, normalized_array[:, 3:]), axis=1
    )
...
def create_position_density_plots(
    x, y, z, x_pr, y_pr, z_pr, bins=100, t=1000, path=""
):
...
def create_momentum_density_plots(
    px, py, pz, px_pr, py_pr, pz_pr, bins=100, t=1000, path=""
):
...
def create_force_density_plots(
    fx, fy, fz, fx_pr, fy_pr, fz_pr, bins=100, t=1000, path=""
):
...
def inference(gpu_index, t_index, hyperparameter_defaults, model):
    ...
    p_gt = load(
        hyperparameter_defaults["pathpattern1"].format(t_index),
        allow_pickle=True,
    )
    ...
    p_rad = from_numpy(
        load(hyperparameter_defaults["pathpattern2"].format(t_index)).astype(
            cfloat
        )
    )
    ...
    create_position_density_plots(
        x, y, z, x_pr, y_pr, z_pr, bins=100, t=t_index
    )

    create_momentum_density_plots(
        px, py, pz, px_pr, py_pr, pz_pr, bins=100, t=t_index
    )

    create_force_density_plots(
        fx, fy, fz, fx_pr, fy_pr, fz_pr, bins=100, t=t_index
    )
